...
function saveState(){try{localStorage.setItem(LS_KEY,JSON.stringify({ACTUALS,SHIPPED}));}catch(e){}}
function loadState(){try{const s=JSON.parse(localStorage.getItem(LS_KEY)||'null');if(s){ACTUALS=s.ACTUALS||{};SHIPPED=s.SHIPPED||{};return true;}}catch(e){}return false;}
...
function renderTest(){
  const rows=BTP.map((b,i)=>`<tr>
    ...
    <td>${b.need?`<select class="field" style="min-width:160px"><option ${b.place.includes('nội')?'selected':''}>Lab QLCL nội bộ</option><option ${b.place==='Test ngoài'?'selected':''}>Test ngoài</option></select>`:'<span class="muted">—</span>'}</td>
    <td class="num">${b.need?`<input class="field" style="width:70px;display:inline-block" value="${b.time}"> giờ`:'<span class="muted">0</span>'}</td>
  ...
  <div class="row">
    <div class="col card" style="min-width:560px">
      <div class="card-h"><h3>Loại bán thành phẩm (BTP)</h3><span class="sub">LAF</span></div>
      <div class="tbl-wrap"><table class="tbl" style="white-space:normal"><thead><tr><th>BTP</th><th>Yêu cầu test</th><th>Nơi test</th><th class="num">Thời gian</th><th>Tiêu chuẩn</th></tr></thead><tbody>${rows}</tbody></table></div>
    </div>
...
window.setBtp=(i,v)=>{BTP[i].need=v;mount('test');};
...
function renderCapacity(){
  const rows=CAP[capFactory].map((p,i)=>`<tr>
    ...
    <td class="num"><input class="field" style="width:90px;display:inline-block" value="${p.cap}" ${capLocked?'disabled':''}> đôi/ngày</td>
  </tr>`).join('');
  return `
  ...
  <div class="row">
    <div class="col card" style="min-width:420px">
      ...
      <div class="card-pad"><div class="field" style="max-width:240px"><label>Thời gian test cộng vào LAF (giờ→ngày)</label><input value="${testBufferDays} ngày" ${capLocked?'disabled':''}></div></div>
    </div>
    <div class="col card" style="flex:0 0 420px;min-width:340px">
      ...
      <div class="card-pad">
        ...
        <div class="sp16"></div>
        <table class="tbl"><thead><tr><th>Ca</th><th>Máy</th><th>Chuyền</th><th class="num">Đôi/giờ</th></tr></thead><tbody>
          <tr><td>Ca 1</td><td>MT-01</td><td>C1</td><td class="num">42</td></tr>
          <tr><td>Ca 1</td><td>MT-02</td><td>C1</td><td class="num">40</td></tr>
          <tr><td>Ca 2</td><td>MT-03</td><td>C2</td><td class="num">38</td></tr>
          <tr><td>Ca 2</td><td>MT-04</td><td>C2</td><td class="num">45</td></tr>
        </tbody></table>
  ...
  <div class="card">
    ...
    <tbody>
      <tr><td><b>Số phom (LVF)</b></td>${SIZES.map(s=>`<td class="num"><input class="field" style="width:52px" value="${[6,10,18,20,14,8,3,0,0,0,0,0][SIZES.indexOf(s)]}" ${capLocked?'disabled':''}></td>`).join('')}<td class="num"><b>89</b></td></tr>
      <tr><td><b>Số khuôn đế (LAF)</b></td>${SIZES.map(s=>`<td class="num"><input class="field" style="width:52px" value="${[4,8,14,16,11,6,2,0,0,0,0,0][SIZES.indexOf(s)]}" ${capLocked?'disabled':''}></td>`).join('')}<td class="num"><b>71</b></td></tr>
    </tbody></table></div>
...
window.lockCap=()=>{capLocked=true;mount('capacity');};
window.unlockCap=()=>{capLocked=false;mount('capacity');};
...
function renderPlan(){
  ...
  <div class="card card-pad" style="margin-bottom:16px">
    <div class="flex wrap" style="gap:18px">
      ...
      <div class="flex wrap">
        <div class="field"><label>Số phom (LVF)</label><input value="89" style="width:90px"></div>
        <div class="field"><label>Số khuôn đế (LAF)</label><input value="71" style="width:90px"></div>
      </div></div>
    </div>
  </div>
